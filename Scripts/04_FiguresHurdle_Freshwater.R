...
#"hu" #####
MaxExtent <- readRDS("Models/FreshwaterMOdels/mod_maxextent_no_rm.rds")
...
for_pl3 <- ce3$`Max_Extent:Reach` %>% 
  ...
  mutate(Max_Extent = case_when(Reach == "Isleta" ~ Max_Extent/86,
                                Reach == "San Acacia" ~ Max_Extent/68))
...
for_pl3 %>% 
  ...
  xlab(expression(paste("Maximum", italic(" Magnitude"))))+
  theme_classic()+
