...
dev.off()
  ...
  #I think this is probability of fish present - "hu"
ce3 <- conditional_effects(m3, effects = "Max_Extent:Species_Codes", dpar="hu")
for_pl3 <- ce3$`Max_Extent:Species_Codes`
...
for_pl3 %>% 
  ...
  ggplot(aes(x = Max_Extent, y = estimate__)) +
  geom_line(color = "blue", size = 1)+
  geom_ribbon(aes(ymin=for_pl3$lower__, ymax=for_pl3$upper__),
              linetype = 2, alpha = 0.1)+
  facet_wrap(vars(Species_Codes), scales = "free", labeller = labeller(Species_Codes= newlabels) )+
  ylab("Hu") + xlab("Max annual extent of dry river miles")+
  theme_classic()
dev.off()
