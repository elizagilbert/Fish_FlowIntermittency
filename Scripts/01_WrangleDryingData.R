...
#Adding 2003 or 2010 to 2019 -2021 ####
dat2003_2018 <- read.csv("Data/Raw/RiverEyesTenthMile2002_2018.csv") %>% 
  select(Year, X) %>% 
...
#bind 2010_2021
dat2003_2021 <- as.data.frame(rbind(dat2019, dat2020, dat2021, dat2003_2018))


  #get all dates and tenths mile to join to data
DatesAll <- as.data.frame(seq(as.Date("2003/1/1"), as.Date("2021/12/31"), "day")) %>% 
  rename(Date = 1)
...
dat2_0 <- FillDatesRM %>% 
  left_join(dat2003_2021, by = c("Date", "RMTenthDry")) %>% 
...
#or for 2003 onwards it is 6,490 days time 1,161 = 8,057,340 in dat2_0

write.csv(dat2_0, "Data/Processed/2003_2021_WetDryTenths.csv")
